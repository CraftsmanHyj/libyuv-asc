package com.hyj.newversion.yuv;
...
import com.hyj.newversion.BuildConfig;
...
public class I420ToBitmapUtils {
    ...
    public I420ToBitmapUtils(Context cxt) {
        this.mContext = cxt;
        this.rs = RenderScript.create(cxt);
     ...
     * 显示当前帧率
     */
    public String showFrameRate() {
        imageCount++;
        if (System.currentTimeMillis() - preTime > 1000) {
            String msg = "被叫 当前帧率：" + imageCount;
            showLog(msg);
...
//        Bitmap bitmap = nv21ToBitmapByYuvImage(nv21Data, width, height);
    ...
    public String saveBitmap(Bitmap mBitmap) {
        File filePic = null;
        try {
            File dir = getSaveDirector(mContext);
            filePic = File.createTempFile(String.valueOf(System.currentTimeMillis()), ".jpg", dir);
     ...
     * @return
     */
    private File getSaveDirector(Context cxt) {
        String storageDir = null;
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            storageDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).getAbsolutePath() + "/0yuv/";
        } else {
            storageDir = cxt.getApplicationContext().getExternalFilesDir(Environment.DIRECTORY_PICTURES).getAbsolutePath() + "/0yuv/";
        }
...
//            imgShow.setImageBitmap(stitchBmp);
    ...
    public void writeToFile(byte[] data) {
        writeToFile("i420Data", ByteBuffer.wrap(data));
    }
    ...
    private void writeToFile(String name, ByteBuffer buffer) {
        try {
            File dir = getSaveDirector(mContext);
//            String filePic = File.createTempFile(name, ".yuv", dir).getAbsolutePath();
//            FileOutputStream fos = new FileOutputStream(filePic, true);

            String path = dir.getAbsolutePath() + "/" + name + ".yuv";
    ...
    /**
     * 将argb的图片转换成i420的byte[]数据
     *
     * @param bitmap
     * @return
     */
    public byte[] argbtoi420(Bitmap bitmap) {
        try {
     ...
     * @return
     */
    public Bitmap i420ToArgbTest(Context cxt) {
        try {
            ...
            byte[] ybuffer = new byte[w * h];//用于保存y分量数据
            byte[] ubuffer = new byte[w * h / 4];//用于保存u分量数据
            byte[] vbuffer = new byte[w * h / 4];//用于保存v分量数据
            //将位图数据argb转换为yuv I420 转换后的数据分别保存在 ybuffer、ubuffer和vbuffer里面
            YuvUtils.argbtoi420(byteArray, w * 4, ybuffer, w, ubuffer, (w + 1) / 2, vbuffer, (w + 1) / 2, w, h);
